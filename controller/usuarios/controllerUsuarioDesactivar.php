...
$id = $_POST['id'];
try {
    $mysql->efectuarConsulta("UPDATE usuarios SET estado_usuario = 'Inactivo' WHERE id_usuario = $id;");
    //? Retorno de datos aplicando JSON
    header('Content-Type: application/json');
    echo json_encode(['success' => true, 'message' => 'Usuario desactivado exitosamente!']);
} catch (\Throwable $th) {
    header('Content-Type: application/json');
    echo json_encode(['success' => false, 'message' => 'Error al desactivar el usuario!', 'error' => $th]);
};
